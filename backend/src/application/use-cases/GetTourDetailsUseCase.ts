...
/**
 * Use-case для получения деталей тура
 */
export class GetTourDetailsUseCase {
  constructor(private readonly mockTourService: MockTourService) {}

  async execute(tourId: string): Promise<TourDto> {
    try {
      logger.info('Getting tour details', { tourId });

      const tour = this.mockTourService.findById(tourId);
      if (!tour) {
      ...
      logger.error('Error getting tour details', error);
      throw error;
    }
  }
