/**
 * Калькулятор риска для автобусов
 * 
 * Учитывает специфичные факторы риска для автобусного транспорта:
...
/**
 * Веса факторов для автобусов
 */
const BUS_FACTOR_WEIGHTS: Record<string, number> = {
  weather: 1.0,
  delays: 0.8,
  cancellations: 1.0,
  transfers: 0.6,
  ...
  seasonality: 0.4,
  occupancy: 0.7,
  scheduleRegularity: 0.8,
};

/**
 * Калькулятор риска для автобусов
 */
export class BusRiskCalculator implements IRiskCalculator {
  ...
  /**
   * Скорректировать веса факторов для автобусов
   * 
  ...
  private adjustFactorWeights(factorResults: IRiskFactorResult[]): IRiskFactorResult[] {
    return factorResults.map((result) => {
      const adjustedWeight = BUS_FACTOR_WEIGHTS[result.description] ?? result.weight;
      return {
        ...result,
