 ...
 * Tour Images Routes
 *
 * API endpoints for tour image management
 */
export function createTourImageRoutes(controller: TourImageController): Router {
  const router = Router();
  ...
  router.post(
    '/:tourId/images',
    upload.array('files', 20), // Maximum 20 files
 ...
 */
export function createStorageRoutes(controller: TourImageController): Router {
  const router = Router();
